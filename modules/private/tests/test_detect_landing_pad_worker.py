...
def main() -> int:
    ...
    for i in range(0, 3):
        input_image_path = pathlib.Path(
            INPUT_IMAGES_PATH, "map_" + str(i) + "_landing_pad.png"
        )
    ...
    for i, camera_image in enumerate(camera_images):
        ...
        # Test
        output_data: "tuple[drone_report.DroneReport, list[bounding_box.BoundingBox], np.ndarray]" = (detect_to_geolocation_queue.queue.get())
